...
export function createPdfDocument(controller: DeliveryFormController) {
  ...
  doc.text(
    'תעודת איסוף – היוזמה הלאומית "מתחשבים-  ציוד תרומה ל "מתחשבים"',
    188,
    70,
    {
      isInputRtl: false,
  ...
  doc.setR2L(true)
  const head = [
    ['תאריך ביצוע האיסוף', 'עיר', 'שם הגוף', 'שם בית חולים/שם המחוז'],
  ]
  const data = [
    [
      doLtr(parseToFullDateStr(d.driverSign?.date!)),
      d.city,
      d.name,
      d.hospitalName,
    ],
  ...
  const data2 = d.items
    ...
    .map((item) => [[doLtr(item.actualQuantity)], [item.name]])
  data2.push([
    [doLtr(d.totalPallets + '')],
    [d.$.totalPallets.metadata.caption],
  ])
